...
def criterio():
    ...
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute('insert into tbl_classificacao_socioeconomica (classificacao) VALUES (%s)', (total))
    conn.commit()
    return render_template('cadastro_criterio.html')
...
def listar_criterio():
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute('select * from tbl_classificacao_socioeconomica')
    data = cursor.fetchall()
    conn.commit()
    return render_template('listar_criterio.html', datas=data)
